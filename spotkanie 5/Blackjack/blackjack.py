...
class BJHand(karty.Hand):
    ...
    @property
    def total(self):
        # jeśli karta w ręce ma wartość None
...
class BJGame(object):
    ...
    def play(self):
        # rozdaj każdemu początkowe 2 karty
        ...
        if not self.still_playing:
            # ponieważ wszyscy garcze dostali furę, pokaż tylko rękę rozdającego
            print(self.dealer)
        else:
            # daj dodatkowe karty rozdajcemu
            print(self.dealer)
            self.__additional_cards(self.dealer)

            if self.dealer.is_busted():
                # wygrywa kjażdy, kto jest jeszcze w grze
                for player in self.still_playing:
                    player.win()
            else:
                # porównaj karty każdego gracza pozostającego w grze z punktami rozdącego
                for player in self.still_playing:
